...
Meduza V is running.
  ...
  API docs     http://127.0.0.1:8000/docs  (Swagger UI грузится с CDN — нужен интернет)
...
* `/docs` (Swagger UI) грузит бандл с CDN, поэтому без интернета страница
  будет пустой. Сам API при этом работает.
* Шрифты подгружаются с Google Fonts; без сети страница отрисуется
  системным шрифтом — на скриншотах выше именно так.
